...
import 'package:json_to_dart/utils/helpers.dart';
...
class TypeDefinition {
  ...
  String toJsonExpression(String key, bool privateField) {
    ...
    if (isPrimitive) {
      ...
      '$key' = $thisKey.map((v) => ${_buildToJsonClass('v')}).toList(),
    }""";
    } else {
      // class
      return """if ($thisKey != null) {
...
class ClassDefinition {
  ...
  String get _jsonParseFunc {
    final sb = new StringBuffer();
    sb.write('factory \t$name.fromJson(String str) => \t$name.fromMap(json.decode(str));\n\n');
    sb.write('String toJson() => json.encode(toMap());\n\n\n');
    sb.write('factory  \t$name');
    sb.write('.fromMap(Map<String, dynamic> json) =>');
    sb.write('\t$name ( \n');
  ...
  String get _jsonGenFunc {
    final sb = new StringBuffer();
    sb.write('\tMap<String, dynamic> toMap() => {\n\t\t');
    fields.keys.forEach((k) {
